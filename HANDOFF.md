# 引き継ぎメモ

## プロジェクト概要

推し活グッズ BtoB ECサイトの要件定義書・画面遷移図・画面設計書・ワイヤーフレームを作成中。

## リポジトリ

- GitHub: https://github.com/Yusuke-holospace/ec-docs (public)
- GitHub Pages: https://yusuke-holospace.github.io/ec-docs/
- ローカル: `/Users/yusukeoniyanagi/Claude Code/ec-docs`

## ファイル構成

```
ec-docs/
├── requirements.md          # 要件定義書（v1.1.0）
├── screen-flow.md            # 画面遷移図（v2.2.0）
├── REVIEW_STATUS.md          # 全画面のレビュー進捗管理
├── screen-design/            # 画面設計書
│   ├── common-header.md      # 共通ヘッダー設計書
│   ├── common-footer.md      # 共通フッター設計書
│   ├── c-01-top.md
│   ├── c-02-product-list.md
│   ├── c-03-product-detail.md
│   ├── c-04-cart.md
│   ├── c-05-upload.md
│   ├── c-06-confirm.md
│   ├── c-07-complete.md
│   ├── c-08-login.md
│   ├── c-09-register.md
│   ├── c-10-mypage.md
│   ├── c-11-profile.md
│   ├── c-12-orders.md
│   ├── c-13-order-detail.md
│   ├── c-14-contact.md
│   ├── c-15-faq.md
│   ├── c-16-guide.md
│   ├── c-17-19-legal.md
│   ├── c-20-404.md
│   └── c-21-status.md
└── wireframes/               # ワイヤーフレーム（HTML）
    ├── c-01-top.html / -sp.html
    ├── c-02-product-list.html / -sp.html
    ├── c-03-product-detail.html / -sp.html
    ├── c-04-cart.html / -sp.html
    ├── c-05-upload.html / -sp.html
    ├── c-06-confirm.html / -sp.html
    ├── c-07-complete.html / -sp.html
    ├── c-08-login.html（PC/SP共通）
    ├── c-09-register.html（PC/SP共通）
    ├── c-10-mypage.html / -sp.html
    ├── c-11-profile.html / -sp.html
    ├── c-12-orders.html / -sp.html
    ├── c-13-order-detail.html / -sp.html
    ├── c-14-contact.html / -sp.html
    ├── c-15-faq.html / -sp.html
    ├── c-16-guide.html / -sp.html
    ├── c-17-terms.html / -sp.html
    ├── c-18-privacy.html / -sp.html
    ├── c-19-legal.html / -sp.html
    ├── c-20-404.html / -sp.html
    └── c-21-status.html / -sp.html
```

## 完了状況

### 購入者向け画面（C-01〜C-20）

| 画面 | PC | SP | 設計書 | 状態 |
|------|----|----|--------|------|
| C-01 トップページ | done | done | done | 完了 |
| C-02 商品一覧ページ | done | done | done | 完了 |
| C-03 商品詳細 | done | done | done | 完了 |
| C-04 カート | done | done | done | 完了 |
| C-05 入稿 | done | done | done | 完了 |
| C-06 注文確認 | done | done | done | 完了 |
| C-07 注文完了 | done | done | done | 完了 |
| C-08 ログイン | done | 共通 | done | 完了 |
| C-09 会員登録 | done | 共通 | done | 完了 |
| C-10 マイページ | done | done | done | 完了 |
| C-11 会員情報 | done | done | done | 完了 |
| C-12 注文履歴 | done | done | done | 完了 |
| C-13 注文詳細 | done | done | done | 完了 |
| C-14 問い合わせ | done | done | done | 完了 |
| C-15 FAQ | done | done | done | 完了 |
| C-16 ご利用ガイド | done | done | done | 完了 |
| C-17 利用規約 | done | done | done | 完了 |
| C-18 プラポリ | done | done | done | 完了 |
| C-19 特商法 | done | done | done | 完了 |
| C-20 404 | done | done | done | 完了 |
| C-21 共有ステータスページ | done | done | done | 完了 |

### 管理者向け画面（A-01〜A-10）

全て**未着手**。画面遷移図（screen-flow.md）に画面一覧と遷移詳細は定義済み。

## 次のタスク

1. ~~**C-04〜C-21の最終チェック**~~ — **完了**（全購入者向け画面のバリデーション完了）
2. ~~**管理者画面（A-01〜A-14）**~~ — **完了**（全管理画面のワイヤーフレーム＋画面設計書作成完了）
3. **エンジニアレビュー** — 全画面をエンジニアに共有し、実装上の懸念点を洗い出す

## 今セッションで実施した主な変更

### 新規作成
- C-01 トップページ（PC/SP版ワイヤー + 画面設計書）
- C-02 商品一覧ページ（PC/SP版ワイヤー + 画面設計書）
- C-21 共有ステータスページ（PC/SP版ワイヤー + 画面設計書）
- common-header.md / common-footer.md（共通コンポーネント設計書）
- REVIEW_STATUS.md（レビュー進捗管理）
- `/validate-wireframe` スキル（最終バリデーション用）

### 要件追加
- 代理注文機能（ADM-011〜012）: EC外注文の管理画面登録
- 共有ステータスページ（ETC-005/007）: トークンURL、C-07/C-13から共有URLコピー
- 仕上がりイメージ（ADM-013/ETC-008）: 販促素材として提供、C-13/C-21でDL
- 担当者割り当て（ADM-014）: 注文単位で担当スタッフ表示（C-13）
- メール通知（NTF-011〜012）: ステータス変更通知、仕上がりイメージ通知
- 注文データフィールド追加: channel, is_proxy_order, status_token

### C-01 トップページの設計変更
- ヒーロー: 無印スタイルのカルーセル（正方形画像、左右2/3表示、無限ループ）
- 商品カテゴリ: 12個、6列×2行、ボーダーなし、ホバーで画像①→②切り替え
- 人気ランキング→人気商品（管理者手動選定、タグ、順位バッジ廃止）
- おすすめ順: おすすめ度5段階+登録日新しい順
- 価格表記: @XX円（税込）〜（ロット10,000個時単価）
- ご利用の流れ/選ばれる理由: Lucideアイコン→画像プレースホルダー
- 制作事例セクション: 削除

### C-03 商品詳細の設計変更
- 商品画像: 80%幅中央寄せ
- サムネイル: 5枚+左右矢印カルーセル
- 単価・最小ロット表示を商品説明直後に追加
- 数量入力をオプション上に移動+クイック選択ボタン
- オプション項目ごとのヘルプアイコン追加
- サンプル制作確認モーダル追加（注意事項+価格+概算納期）
- サブリンクをLucideアイコン化
- 管理画面のオプション設定項目一覧を設計書に追記

### C-13 注文詳細の設計変更
- 担当スタッフ表示セクション追加
- 仕上がりイメージDLセクション追加（商品一覧直後に配置）
- 共有URLコピー: 独立セクション→ページタイトル横のシェアボタンに変更
- 再入稿ボタン: ページ上部アラート→差し戻された商品行に個別表示

### ヘッダー・フッターの全画面統一（32ファイル）
- 告知バー追加（PC版のみ）
- ロゴ画像プレースホルダー化
- 検索窓センター固定+角丸+アイコン内蔵
- ユーティリティリンクをLucideアイコン縦積み
- メガメニュー全幅展開+画像ホバー切り替え
- SP版: ドロワーメニュー（カテゴリ一覧アコーディオン）追加
- フッター: ロゴ画像+サポート+サイト情報（運営会社）

## 重要な設計ルール

### PCワイヤーフレーム
- ヘッダー2段構成: 1段目（ロゴ・検索・ユーティリティリンク・カートアイコン）、2段目（カテゴリナビ・ホバーでドロップダウン）
- 告知バー: ヘッダー最上部に黒背景の告知バー「デザインデータをそのまま送るだけ。入稿データの編集はスタッフが無料でサポートします。」
- ロゴ: 画像プレースホルダー（120×36px）
- 検索窓: センター固定（position: absolute）、角丸（border-radius: 20px）、アイコン内蔵
- ユーティリティリンク: Lucideアイコン＋テキストの縦積み（ご利用ガイド、FAQ、入稿について、ログイン、カート）
- カテゴリナビ: ホバーでメガメニュー全幅展開（4列グリッド）。ホバー時の下線なし
- メガメニュー画像: ホバーで①→②切り替え（opacity transition 0.3s）
- パンくずリスト: ヘッダー下＋フッター直上（隙間なし）
- ログイン/マイページ: 未ログイン「ログイン」、ログイン済み「マイページ」に切替
- カートアイコン: プレースホルダー（iconテキスト）＋バッジ
- フッター: ロゴ画像（240×72px）+ サポート + サイト情報の3カラム。「運営会社」リンクを最上部に配置
- アイコンライブラリ: Lucide（lucide-react）
- 価格表記: @XX円（税込）〜

### SPワイヤーフレーム
- max-width: 390px
- 告知バーなし
- ロゴ: 画像プレースホルダー（90×28px）
- ヘッダー: ロゴ＋ログイン/カート/メニュー（アイコン＋テキスト）＋検索バー
- ナビアイコン: Lucideアイコン（ログイン、カート、メニュー）
- 検索窓: 角丸（border-radius: 20px）、アイコン内蔵
- ドロワーメニュー: 右からスライド、カテゴリ一覧はアコーディオン
- パンくずなし
- タップ領域: 全要素44px以上
- ドラッグ&ドロップなし（ファイル選択ボタンのみ）
- タブ → アコーディオン
- 関連商品: グリッド → 横スクロール
- フッター: ロゴ画像（180×54px）+ サポート + サイト情報の縦積み
- 絵文字使用禁止

### 共通
- アイコンはプレースホルダー（「icon」テキスト）
- 価格は税込表示
- 送料は商品ごとに設定、カートで合算、N円以上で無料
- カートに追加はログイン不要、入稿・注文確定時にログイン必要
- お客様情報登録はスキップ可能、入稿・注文確定時に未登録ならC-11に遷移
- サンプル制作: 1式（10個）、式数変更可能
- 入稿スロット: 1スロット=1ファイル、オプション連動で表示/非表示
- ライセンス同意: C-06（注文確認ページ）で1回のみ
- 支払い方法: クレカ（Stripe）、銀行振込、請求書払い。C-06で選択
- 決済はこの時点では発生しない旨を複数箇所に明記

## 参考サイト
- Casetify（カスタム商品EC）
- 無印良品（シンプルなデザイン）
- Amazon（UXの基準）
- 販促スタイル（同業種のBtoB EC）
