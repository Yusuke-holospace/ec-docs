...
| C-21 共有ステータスページ | done | done | done | 完了 |
...
1. **管理者画面（A-01〜A-11）** — ワイヤーフレーム＋画面設計書
2. **購入者向け画面（C-03〜C-20）の最終チェック** — `/validate-wireframe` による最終バリデーション

## 重要な設計ルール

### PCワイヤーフレーム
- ヘッダー2段構成: 1段目（ロゴ・検索・ユーティリティリンク・カートアイコン）、2段目（カテゴリナビ・ホバーでドロップダウン）
- 告知バー: ヘッダー最上部に黒背景の告知バー「デザインデータをそのまま送るだけ。入稿データの編集はスタッフが無料でサポートします。」
- ロゴ: 画像プレースホルダー（120×36px）
- 検索窓: センター固定（position: absolute）、角丸（border-radius: 20px）、アイコン内蔵
- ユーティリティリンク: Lucideアイコン＋テキストの縦積み（ご利用ガイド、FAQ、入稿について、ログイン、カート）
- カテゴリナビ: ホバーでメガメニュー全幅展開（4列グリッド）。ホバー時の下線なし
- メガメニュー画像: ホバーで①→②切り替え（opacity transition 0.3s）
- パンくずリスト: ヘッダー下＋フッター直上（隙間なし）
- ログイン/マイページ: 未ログイン「ログイン」、ログイン済み「マイページ」に切替
- カートアイコン: プレースホルダー（iconテキスト）＋バッジ
- フッター: ロゴ画像（240×72px）+ サポート + サイト情報の3カラム。「運営会社」リンクを最上部に配置
- アイコンライブラリ: Lucide（lucide-react）
- 価格表記: @XX円（税込）〜

### SPワイヤーフレーム
- max-width: 390px
- 告知バーなし
- ロゴ: 画像プレースホルダー（90×28px）
- ヘッダー: ロゴ＋ログイン/カート/メニュー（アイコン＋テキスト）＋検索バー
- ナビアイコン: Lucideアイコン（ログイン、カート、メニュー）
- 検索窓: 角丸（border-radius: 20px）、アイコン内蔵
- ドロワーメニュー: 右からスライド、カテゴリ一覧はアコーディオン
- パンくずなし
- タップ領域: 全要素44px以上
- ドラッグ&ドロップなし（ファイル選択ボタンのみ）
- タブ → アコーディオン
- 関連商品: グリッド → 横スクロール
- フッター: ロゴ画像（180×54px）+ サポート + サイト情報の縦積み
- 絵文字使用禁止
